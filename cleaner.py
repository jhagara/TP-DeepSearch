...
# purpose of this class is simply in cleaning the input xml, then this cleaned xml is used in further steps
# of collecting and assembling articles from headings, fulltexts and separators
class Cleaner(object):
    DOC_ATTRS = ['producer', 'version', 'pagesCount', 'mainLanguage', 'languages']
    ...
    def clean(cls, parsed_xml):
        ...
        # replace hierarchy: block -> text -> par -> line, with new one: block -> par -> line
        ...
        # remove unnecessary element par from block
        for region in parsed_xml.xpath('//region'):
            region.getparent().remove(region)

        print(etree.tostring(parsed_xml))
        return parsed_xml
    ...
    # depending on node tag name, use corresponding desired attributes and push them into method for
